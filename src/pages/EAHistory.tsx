...
import { Code2, Clock, ChevronDown, ChevronUp, Copy, Pencil, Trash2, X, Check, Calendar, Loader2, AlertCircle } from 'lucide-react';
...
export function EAHistory() {
    ...
    const unsubscribe = onValue(userHistoryRef, (snapshot) => {
      try {
        ...
        const historyItems = Object.entries(data)
          ...
          .filter(item => item.type === 'ea-generator')
          .sort((a, b) => 
  ...
  const handleEdit = async (item: HistoryItem) => {
    ...
    try {
      ...
      setEditTitle(item.title || 'Expert Advisor');
      setError(null);
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} p-6 md:p-12 lg:p-24`}
         style={{
           background: `
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold flex items-center app-text">
            <Code2 className="mr-2 app-icon" /> EA Generator History
          </h1>
        ...
        {!loading && !error && filteredHistory.length === 0 && (
          ...
          style={theme === 'light' ? {
            ...
            <Code2 className={`w-12 h-12 mx-auto mb-4 ${theme === 'dark' ? 'text-cyan-600' : 'text-cyan-400'}`} 
            ...
            <p className={`mb-4 ${theme === 'dark' ? 'text-cyan-500' : 'text-cyan-500'}`}>
              {startDate || endDate
                ? "No Expert Advisors match your date filter. Try adjusting your filter criteria."
                : "You haven't created any Expert Advisors yet. Start by generating a new EA!"}
            </p>
            {(startDate || endDate) && (
              <Button
                variant="outline"
      ...
      {deleteItem && (
        <DeleteConfirmationModal
          ...
          message={`Are you sure you want to delete "${deleteItem.title || 'this Expert Advisor'}"? This action cannot be undone.`}
        />
      )}
      ...
      {showClearConfirmation && (
        <DeleteConfirmationModal
          ...
          message="Are you sure you want to delete all your EA Generator history? This action cannot be undone."
        />
      )}
